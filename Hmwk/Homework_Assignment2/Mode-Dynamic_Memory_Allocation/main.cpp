 ...
 * File:   main.cpp
 * Author: Jacob Gower
 * Created on March 6th, 2021, 7:45 PM
 * Purpose: To find the mode of a set of numbers
 */
...
//Create and fill a dynamic array with mode values
int *mode(const int *array,int arySize){
    int count=0, //Counter for frequency
       frqncy=0, //Max frequency
       nmodes=0; //Number of modes
    ...
    for(int i=0;i<arySize;i++){
        ...
        if(count>frqncy){
            //If counter is larger than current max frequency, set frequency equal to counter
            frqncy=count;
        }
    }
    //Reset counter
    ...
    //For loop for the same array but now comparing count value to frequency to find number of modes
    for(int i=0;i<arySize;i++){
        //Iterate counter the same way
        if(i==0){
            count++;
        }else if(*(ary+i)==*(ary+(i-1))){
            //If match is detected, increment counter
        ...
        if(count==frqncy&&count>1){
            //Increment number of modes
            nmodes++;
        }
    }
    //Allocate the mode array
    int *modeAry=new int[nmodes+2];//Use number of modes + 2 to size array
    //[0] position in array is equal to number of modes
    modeAry[0]=nmodes;
    //[1] position in array is equal to max frequency
    modeAry[1]=frqncy;
    //Reset counter
    count=0;
    ...
    //Fill the mode array
    //Use the same loop because it needs to pull modes from the array
    for(int i=0,j=2;i<arySize;i++){
        ...
        if(count==frqncy&&count>1){
            //Use separate counter for modeAry since the positions in the different arrays will not necessarily correspond
            modeAry[j]=ary[i];
            //Increment the modeAry position
            j++;
        }
    }
